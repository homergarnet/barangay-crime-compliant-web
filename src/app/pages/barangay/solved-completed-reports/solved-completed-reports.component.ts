...
@Component({
  selector: 'app-solved-completed-reports',
  templateUrl: './solved-completed-reports.component.html',
  styleUrls: ['./solved-completed-reports.component.scss']
})
export class SolvedCompletedReportsComponent implements OnInit {
  ...
  initialReceiveCrimeList(currentPageVal: number = 1, resultPerPageVal: number = 10) {
    ...
    this.receiveCrimeService.receiveCrimeList('','completed','',currentPageVal == 0? this.currentPage : currentPageVal,resultPerPageVal == 0 ? this.resultPerPage : resultPerPageVal).subscribe(
      (res) => {

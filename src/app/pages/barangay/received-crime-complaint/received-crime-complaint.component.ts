...
@Component({
  selector: 'app-received-crime-complaint',
  templateUrl: './received-crime-complaint.component.html',
  styleUrls: ['./received-crime-complaint.component.scss']
})
export class ReceivedCrimeComplaintComponent implements OnInit {
  ...
  initialReceiveCrimeList(currentPageVal: number = 1, resultPerPageVal: number = 10) {
    ...
    this.receiveCrimeService.receiveCrimeList('compliant','not closed completed','',currentPageVal == 0? this.currentPage : currentPageVal,resultPerPageVal == 0 ? this.resultPerPage : resultPerPageVal).subscribe(
      (res) => {
